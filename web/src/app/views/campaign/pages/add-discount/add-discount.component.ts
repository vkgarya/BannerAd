...
export class AddDiscountComponent implements OnInit {
  ...
  onAddCouponDescription(): void {
    ...
    (this.discountForm.get('coupon').get('couponDescriptions') as FormArray).push(couponDescription);
  }

  onDeleteCouponDescription(couponDescriptionIndex: number): void {
    (this.discountForm.get('coupon').get('couponDescriptions') as FormArray).removeAt(couponDescriptionIndex);
  }
  ...
  onAddCouponCode(): void {
    ...
    (this.discountForm.get('coupon').get('couponCodes') as FormArray).push(couponCode);
  }

  onDeleteCouponCode(couponCodeIndex: number): void {
    (this.discountForm.get('coupon').get('couponCodes') as FormArray).removeAt(couponCodeIndex);
  }

  so() {
    console.log(this.discountForm.get('coupon'));
  }

  onSelectCouponType(): void {
    let couponType = this.discountForm.get('coupon').get('couponType').value;

    if (this.discountForm.get('coupon').get('couponValue')) {
      (this.discountForm.get('coupon') as FormGroup).removeControl('couponValue');
    }

    if (couponType === 'amount') {
      (this.discountForm.get('coupon') as FormGroup).addControl('couponValue', new FormGroup({
